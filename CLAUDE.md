...
- **crimson**（本项目）：用户认证、分享管理、分析数据展示、截图文件存储与服务
- **crimson-render**（独立服务）：渲染用户上传的 HTML 内容 + 采集浏览数据
- **crimson-screenshot**（独立服务）：Playwright 截图生成，运行在 Mac 上

crimson 与 crimson-render 拆分的核心原因是**安全**——用户上传的 HTML 可能包含恶意脚本，渲染服务部署在不同域名，通过浏览器同源策略实现沙箱隔离。crimson-screenshot 独立是因为 Playwright 对香橙派 ARM 负载太重，部署在 Mac（固定内网 IP）上。
...
- 三种访问控制：公开 / 密码保护 / 私有
- 生成一次性 token 供 crimson-render 验证访问
...
2. **生成**：`crimson-screenshot` 服务消费队列，Playwright 渲染后 POST 图片回 crimson
...
| 路由 | 方法 | 说明 |
|------|------|------|
| `/api/createScreenShot` | POST | 接收截图（仅 crimson-screenshot 调用，X-Screenshot-Secret 验证） |
